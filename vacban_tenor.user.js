...
// @version      1.0
// @description  Tenor GIF Search with Favorites, Categories and Hover Preview for VacBan.wtf
// @author       AMIAY_seg
// @match        *://vacban.wtf/*
// @grant        none
...
(function() {
    ...
    styleSheet.innerText = "" +
        "#tenor-popup { display: none; position: absolute; z-index: 10000; width: 340px; background: #2c2c2c; border: 1px solid #444; border-radius: 4px; box-shadow: 0 5px 15px rgba(0,0,0,0.5); padding: 0; font-family: inherit; color: #d1d1d1; overflow: hidden; }\n" +
        "#tenor-header { background: #353535; padding: 8px 12px; font-weight: bold; font-size: 13px; border-bottom: 1px solid #444; display: flex; justify-content: space-between; align-items: center; color: #fff; }\n" +
        "#tenor-back { cursor: pointer; color: #00b0ff; margin-right: 10px; display: none; }\n" +
        "#tenor-search-container { padding: 10px; border-bottom: 1px solid #333; }\n" +
        "#tenor-search-input { width: 100%; padding: 7px 10px; border-radius: 3px; border: 1px solid #444; background: #1a1a1a; color: white; box-sizing: border-box; outline: none; font-size: 12px; }\n" +
        "#tenor-results { display: grid; grid-template-columns: repeat(2, 1fr); gap: 6px; max-height: 380px; overflow-y: auto; padding: 10px; }\n" +
        ".tenor-item { position: relative; width: 100%; height: 95px; border-radius: 3px; overflow: hidden; cursor: pointer; background: #1a1a1a; }\n" +
        ".tenor-gif { width: 100%; height: 100%; object-fit: cover; transition: transform 0.2s; }\n" +
        ".tenor-item:hover .tenor-gif { transform: scale(1.05); }\n" +
        ".tenor-fav-btn { position: absolute; top: 5px; right: 5px; color: #fff; font-size: 16px; opacity: 0; transition: opacity 0.2s, color 0.2s; text-shadow: 0 0 3px rgba(0,0,0,0.8); z-index: 5; }\n" +
        ".tenor-item:hover .tenor-fav-btn { opacity: 1; }\n" +
        ".tenor-fav-btn.active { opacity: 1; color: #ffeb3b; }\n" +
        ".tenor-cat-tile { position: relative; height: 75px; border-radius: 3px; overflow: hidden; cursor: pointer; background: #333; }\n" +
        ".tenor-cat-overlay { position: absolute; inset: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; text-align: center; font-size: 11px; font-weight: bold; color: #fff; transition: background 0.2s; text-transform: capitalize; padding: 5px; }\n" +
        ".tenor-cat-tile:hover .tenor-cat-overlay { background: rgba(0,0,0,0.3); }\n" +
        "#tenor-big-preview { display: none; position: fixed; z-index: 10001; width: 220px; height: auto; background: #1a1a1a; border: 2px solid #00b0ff; border-radius: 5px; box-shadow: 0 10px 25px rgba(0,0,0,0.8); pointer-events: none; overflow: hidden; }\n" +
        "#tenor-big-preview img { width: 100%; height: 100%; display: block; }\n" +
        ".tenor-message { text-align: center; color: #888; font-size: 11px; grid-column: 1 / -1; padding: 15px 0; }\n" +
        "#tenor-results::-webkit-scrollbar { width: 4px; }\n" +
        "#tenor-results::-webkit-scrollbar-thumb { background: #444; border-radius: 2px; }";
    ...
    const renderCategories = () => {
        ...
        favTile.style.background = '#00b0ff';
        favTile.innerHTML = `<div class="tenor-cat-overlay" style="background: rgba(0,176,255,0.6)">Favorites ★</div>`;
